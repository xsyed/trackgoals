...
class Friend{
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, firstname, photo FROM `friend_request` JOIN users ON friend_request.sender = users.id WHERE receiver = ?";
            ...
            if($send_data){
                ...
                foreach($all_users as $row){
                    if($row->user_one == $my_id){
                        $get_user = "SELECT id, firstname,lastname,email,photo FROM `users` WHERE id = ?";
